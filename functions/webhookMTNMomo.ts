...
Deno.serve(async (req) => {
  try {
    ...
    console.log('Webhook MTN MoMo reçu:', webhookData);

    const { externalId, status, financialTransactionId } = webhookData;
    ...
    if (status === 'SUCCESSFUL') {
      await processSuccessfulPayment(base44, transaction, financialTransactionId);
      ...
      await base44.asServiceRole.entities.Transaction.update(transaction.id, {
        ...
        metadata: {
          ...
          failure_reason: 'Paiement MTN échoué'
        }
      });
...
async function processSuccessfulPayment(base44, transaction, financialTransactionId) {
  // 1. Mettre à jour transaction
  await base44.asServiceRole.entities.Transaction.update(transaction.id, {
    payment_status: 'completed',
    payment_confirmed_at: new Date().toISOString(),
    external_transaction_id: financialTransactionId,
    metadata: {
      ...transaction.metadata,
      webhook_received_at: new Date().toISOString()
    }
